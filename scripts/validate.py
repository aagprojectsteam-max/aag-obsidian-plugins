...
import json,re
root=Path(__file__).resolve().parents[1];data=json.loads((root/'catalog.json').read_text());plugins=data['plugins'];assert len(plugins)==2
assert len({p['id'] for p in plugins})==len(plugins)
for p in plugins:
 assert re.fullmatch(r'aagprojectsteam-max/aag-obsidian-(designtweaker|smartpaste)',p['repository'])
